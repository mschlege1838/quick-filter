...
export function idHelper(el, suffix) {
    for (const child of el.children) {
        if (child.id) {
            child.id += suffix;
        }
        if (child.htmlFor) {
            child.htmlFor += suffix;
        }
        if (child.name) {
            child.name += suffix;
        }
        idHelper(child, suffix);
    }
}
...
export class SimpleEventDispatcher extends SimpleEventDispatcherMixin(Object) {
    
}

export class IdAwareElement extends HTMLElement {
    ...
    connectedCallback() {
        ...
        idHelper(fragment, IdAwareElement.idCounter++);
        
        const attachRef = this.attachShadow({mode: 'open'});
        if (this.stylesheet) {
            attachRef.adoptedStyleSheets.push(this.stylesheet);
        }
        while (fragment.childNodes.length) attachRef.appendChild(fragment.childNodes[0]);
